...
export default function Funders({ grants, stages, onSelectGrant }) {
  // Card expand / owner-dropdown open-close (transient UI state, stays local)
  const [expandedFunder, setExpandedFunder] = useState(null);

  // View-model: grouping, filter, stats, and the funder-owner KV store
  const {
    filtered, stats,
    owners: funderOwners, assignOwner: assignFunderOwner,
      ...
      {filtered.length === 0 && (
        <div style={{ textAlign: "center", padding: "40px 20px", color: C.t3 }}>
          ...
          <div style={{ fontSize: 13, color: C.t4 }}>
            ...
            {" "}
            <button onClick={clearFilters} style={{ color: C.primary, background: "none", border: "none", cursor: "pointer", fontWeight: 600, fontFamily: FONT, fontSize: 13 }}>Clear filters</button>
          </div>
        </div>
      )}
    </div>
