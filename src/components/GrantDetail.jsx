...
import { Card, Hd, Field, ActivityRow, ContextSidebar, StatusStrip, StageBanner } from "./GrantDetailParts";
...
export default function GrantDetail({ grant, team, stages, funderTypes, complianceDocs = [], currentMember, orgName = "the organisation", onUpdate, onDelete, onAddGrant, onSelectGrant, onBack, onRunAI, onUploadsChanged, onLaunchTour }) {
  ...
  useEffect(() => {
    if (!overflow) return;
    const handler = (e) => { if (overflowRef.current && !overflowRef.current.contains(e.target)) setOverflow(false); };
  ...
  // Load uploads when grant detail opens
  useEffect(() => {
    if (!uploadsLoaded.current) loadUploads();
  }, [loadUploads]);
  ...
  return (
    <div style={{ padding: "16px 16px", maxWidth: 920 }}>
      {/* Persistent context sidebar — only visible on wide screens (>1320px). Lifted to
          GrantDetailParts (props-only). */}
      <ContextSidebar g={g} stages={stages} complianceDocs={complianceDocs} />
      ...
      {/* Fixed status strip — key facts always visible while scrolling. Lifted to
          GrantDetailParts (props-only). */}
      <StatusStrip g={g} team={team} stg={stg} complianceDocs={complianceDocs} />

      {/* Stage-aware "what now?" banner — lifted to GrantDetailParts (props-only). */}
      <StageBanner g={g} complianceDocs={complianceDocs} />
          ...
          <div style={{ display: "flex", gap: 8, flexShrink: 0, position: "relative" }} ref={overflowRef}>
            ...
            <button onClick={() => setOverflow(p => !p)}
              style={{
                ...
                background: overflow ? C.raised : "transparent", cursor: "pointer",
                display: "flex", alignItems: "center", justifyContent: "center",
                fontSize: 16, color: C.t3, fontFamily: FONT, transition: "background 0.15s ease",
              }}
              title="More actions"
            >{"\u22EF"}</button>
            {overflow && (
              ...
              <>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginTop: 6, flexWrap: "wrap" }}>
                  ...
                  <button onClick={() => { setAskInput(String(g.ask)); setEditingAsk(true); }}
                    ...
                    style={{ fontSize: 12, color: C.t4, background: "none", border: "none", cursor: "pointer", padding: 0, lineHeight: 1 }}>
                    {"\u270E"}
                  </button>
                </div>
              </>
          ...
          <div style={{ marginBottom: 16, padding: "8px 14px", borderRadius: 8, background: C.warm100, border: `1px solid ${C.line}` }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 6 }}>
              ...
              <div style={{ flex: 1, height: 4, borderRadius: 2, background: C.line, overflow: "hidden" }}>
                <div style={{ height: "100%", width: `${r.score}%`, background: barColor, borderRadius: 2, transition: "width 0.4s ease" }} />
              </div>
              {r.missing.length > 0 && (
                <span style={{ fontSize: 10, fontWeight: 600, color: C.amber, background: C.amberSoft, padding: "2px 8px", borderRadius: 6, whiteSpace: "nowrap" }}
                  title={r.missing.join(", ")}
                >{r.missing.length} missing</span>
              )}
            </div>
            {r.nextAction && (
      ...
      {(() => {
        ...
        const steps = [
          { key: "fitscore", label: "Fit Score", done: fitDone, busy: busy.fitscore, value: fitScoreNum ? `${fitScoreNum}` : null, color: fitDone ? (fitScoreNum >= 70 ? C.ok : fitScoreNum >= 40 ? C.amber : C.red) : C.t4 },
      ...
      {/* ══ Stage-Aware Content Sections ══ */}

      {/* Grant Profile — EARLY, CLOSED, or showAll */}
      {(showAll || isEarly || isClosed) && (
        <SectionWrap title="Grant Profile" defaultOpen={isEarly || isClosed}>
          <Card>
      ...
      {/* Fit Score — EARLY (open), MIDDLE (collapsed badge), or showAll */}
      {(showAll || isEarly || isMiddle) && (
        <SectionWrap
          title="Fit Score"
          defaultOpen={isEarly || !fitDone}
          badge={fitDone && fitScoreNum !== null ? (
            <span style={{
              fontSize: 11, fontWeight: 700, fontFamily: MONO, padding: "2px 8px", borderRadius: 6,
              background: fitScoreNum >= 70 ? C.okSoft : fitScoreNum >= 40 ? C.amberSoft : C.redSoft,
              color: fitScoreNum >= 70 ? C.ok : fitScoreNum >= 40 ? C.amber : C.red,
            }}>{fitScoreNum} — {fitVerdict || "Scored"}</span>
          ) : null}
        >
          <div style={{
          ...
          {fitDone && (
            <details style={{ marginBottom: 14 }}>
              <summary style={{ fontSize: 12, fontWeight: 600, color: C.t3, cursor: "pointer", padding: "4px 0", userSelect: "none" }}>
                View full analysis
              </summary>
              <div style={{
          ...
          {g.fitscoreHistory && g.fitscoreHistory.length > 0 && (
            <details style={{ fontSize: 12, color: C.t3, marginTop: -8 }}>
      ...
      {/* Funder Research — EARLY (open), MIDDLE (collapsed), or showAll */}
      {(showAll || isEarly || isMiddle) && (
        <SectionWrap
          title="Funder Research"
          defaultOpen={isEarly || !resDone}
          badge={resDone ? (
            <span style={{ fontSize: 11, fontWeight: 600, color: C.ok, background: C.okSoft, padding: "2px 8px", borderRadius: 6 }}>
              {"\u2713"} Done {g.aiResearchAt ? `\u00B7 ${timeAgo(g.aiResearchAt)}` : ""}
            </span>
          ) : null}
        >
          <AICard
          ...
          {g.researchHistory && g.researchHistory.length > 0 && (
            <details style={{ fontSize: 12, color: C.t3, marginTop: 6 }}>
              <summary style={{ cursor: "pointer", fontWeight: 600, padding: "6px 0", userSelect: "none" }}>
      ...
      {(showAll || isLate) && g.fups && g.fups.length > 0 && (
        <SectionWrap title="Follow-ups" defaultOpen={isLate}>
      ...
      {/* Follow-up Email — MIDDLE, LATE, or showAll */}
      {(showAll || isMiddle || isLate) && (
        <SectionWrap title="Follow-up Email" defaultOpen={isLate}>
          <AICard
      ...
      {/* Docs Checklist — MIDDLE (collapsed), LATE (open), or showAll */}
      {(showAll || isMiddle || isLate) && (() => {
        const required = DOCS[g.type];
        if (!required) return null;
        const orgDocs = [];
        ...
        for (const docName of required) {
          ...
          if (orgDocId) {
            ...
            let statusLabel = "Missing", statusColor = C.red, statusBg = C.redSoft, statusIcon = "✗";
            if (cd && (cd.status === "valid" || cd.status === "uploaded")) {
              if (dl !== null && dl <= 0) { statusLabel = "Expired"; statusColor = C.red; statusBg = C.redSoft; statusIcon = "✗"; }
              else if (dl !== null && dl <= 30) { statusLabel = `Expires in ${dl}d`; statusColor = C.amber; statusBg = C.amberSoft; statusIcon = "⚠"; }
              else { statusLabel = "Valid"; statusColor = C.ok; statusBg = C.okSoft; statusIcon = "✓"; }
            } else if (cd?.status === "expired") { statusLabel = "Expired"; statusColor = C.red; statusBg = C.redSoft; statusIcon = "✗"; }
            orgDocs.push({ docName, orgDocId, orgDocDef, statusLabel, statusColor, statusBg, statusIcon });
          } else {
      ...
      {/* Attachments — LATE (open), MIDDLE (collapsed), or showAll */}
      {(showAll || isMiddle || isLate) && (
        <SectionWrap title={`Attachments${uploads.length ? ` (${uploads.length})` : ""}`} defaultOpen={isLate}>
          <UploadZone
            uploads={uploads}
            grantId={g.id}
      ...
      {/* Win/Loss Analysis — CLOSED only */}
      {(showAll || isClosedStage) && (
        <SectionWrap title={g.stage === "won" ? "Win Analysis" : "Loss Analysis"} defaultOpen={isClosedStage}>
          <AICard
      ...
      <div style={{ textAlign: "center", marginTop: 8, marginBottom: 24 }}>
        <button onClick={() => setShowAll(p => !p)}
          style={{
            fontSize: 12, fontWeight: 600, color: C.t3, background: "none",
            border: `1px solid ${C.line}`, borderRadius: 8, padding: "6px 16px",
            cursor: "pointer", fontFamily: FONT, transition: "color 0.15s ease",
          }}
          onMouseEnter={e => e.currentTarget.style.color = C.primary}
          onMouseLeave={e => e.currentTarget.style.color = C.t3}
        >
