// Prompt builders for pipeline-wide action types (brief, report, insights,
// strategy), extracted verbatim from useAI.js. Pure functions returning
// { system, user, search, maxTokens }. The data-aggregation that prepares the
// user prompt lives here (it only ever feeds prompt-string construction).
import { dL, effectiveAsk } from "../../utils";
import { detectType, PTYPES } from "../../data/funderStrategy";

export function buildBrief(ctx) {
  const { grants, team, orgName } = ctx;
  ...
  return {
    system: `You are the grant operations manager for ${orgName}. Produce a daily action list — the 5-8 things that will move the pipeline forward TODAY.
...
- End with a one-line pipeline health summary`,
    user: `Today: ${new Date().toLocaleDateString("en-ZA", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}
...
export function buildReport(ctx) {
  const { grants, stages, orgCtx, orgName, factGuard } = ctx;
  ...
  const byStage = stages.filter(s => !["won", "lost", "deferred", "archived"].includes(s.id))
    ...
    .join(", ");
  return {
    system: `You write quarterly impact reports for ${orgName}'s funders. Audience: existing funders and board members who want to see progress, outcomes, and pipeline health.
...
// Mirrors the original inline aggregation in useAI.js exactly.
function aggregatePipeline(ctx) {
  const { grants, team } = ctx;
  const act = [], won = [], lost = [];
  ...
  return { act, won, lost, funderTypeMap, relMap, focusMap, ownerMap, stageCounts, totalAsk, wonVal, withAI, deadlinePressure, overdueCount, noDeadline, closed };
}

export function buildInsights(ctx) {
  const { grants, stages, orgName, factGuard } = ctx;
  const { act, won, lost, funderTypeMap, relMap, focusMap, ownerMap, stageCounts, totalAsk, wonVal, withAI, closed, deadlinePressure, overdueCount, noDeadline } = aggregatePipeline(ctx);
  ...
  return {
    system: `You are a sharp-eyed pipeline analyst for ${orgName}. You find the things that busy grant managers miss — the hidden risks, the unexploited patterns, the signals in the noise.
...
export function buildStrategy(ctx) {
  const { grants, orgCtx, orgName, factGuard } = ctx;
  const { act, won, lost, funderTypeMap, totalAsk, wonVal } = aggregatePipeline(ctx);
  ...
  return {
    system: `You are a funding strategist for ${orgName}. The organisation's full context is provided below.
