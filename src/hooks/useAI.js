...
LOST GRANTS: ${lost.map(g => `${g.name} from ${g.funder} (${g.type}, R${effectiveAsk(g).toLocaleString()})`).join("; ") || "None yet"}`,
    ...
    if (type === "rewriteForReadability") {
      const { sentences = [], target = 50 } = priorResearch || {};
      if (!Array.isArray(sentences) || !sentences.length) return "[]";
      const numbered = sentences.map((s, i) => `${i + 1}. ${s}`).join("\n");
      return await api(
        `You rewrite individual sentences from a funding proposal to be clearer and easier to read, WITHOUT changing their meaning.
...
- If a sentence is already clear, return it unchanged.`,
        `Rewrite each of these ${sentences.length} sentences to read more easily (Flesch target ~${target}). Return a JSON array of exactly ${sentences.length} strings, in order:\n\n${numbered}`,
        false, 1200
      );
    }
